...
# Google API Key
GOOGLE_API_KEY = "AIzaSyCvI2QO5qv9oKFj5AKuREFRePzUtgGk4fQ" 
...
#Inserting vehicle details into MySQL database
...
class SpeedEstimator(BaseSolution):
    ...
    # Speed Estimation
    def estimate_speed_from_detections(self, im0, detections):
        """
        Estimate speed using externally provided detections (e.g., from Roboflow or other inference engine).
        """
        self.annotator = Annotator(im0, line_width=self.line_width)
        
        # Draw region polygon
        pts = np.array(self.region, np.int32).reshape((-1, 1, 2))
        cv2.polylines(im0, [pts], isClosed=True, color=(104, 0, 123), thickness=self.line_width * 2)

        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # Extract data from detections object
        boxes = detections.xyxy  # Bounding boxes (N x 4)
        track_ids = detections.tracker_id  # Tracker IDs (N)
        class_ids = detections.class_id  # Class IDs (N)
        class_names = detections.data.get('class_name', ["vehicle"] * len(track_ids))
        ...
        for i, (box, track_id, cls_id) in enumerate(zip(boxes, track_ids, class_ids)):
            ...
            if direction == "known" and track_id not in self.trkd_ids:
                ...
                if time_difference > 0:
                    ...
                    speed = round(pixel_distance / time_difference)  # Add scaling if needed
...
# Mouse Callback for printing x, y coodinates when mouse moves over the OpenCV window
